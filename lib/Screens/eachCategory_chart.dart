...
class EachCategory_Chart extends StatefulWidget {
  ...
  const EachCategory_Chart({super.key, required this.category});
  @override
    ...
    return Scaffold(
        ...
        leading: IconButton(
          ...
          onPressed: () {
            Navigator.of(context).pushReplacement(
              MaterialPageRoute(
                  builder: (context) => ExpensiveScreen(category: widget.category,)),
            );
          },
        ),
      ),
      body: Container(
        child: FutureBuilder<List<Expense>>(
            future: getExpense.fetchExpenses(widget.category!.sId!),
            builder: (context, snapshot) {
              if (!snapshot.hasData) {
                ...
                return Center(
                    child:
                        Text('No expenses found, Add it to view it in chart.'));
              }
